function GameWorld() {
	...
	this.start = function (stateParams) {
		this.setWaitStatus('La partie va commencer...');
	}

	this.starting = function (stateParams) {
		this.setWaitStatus("C'est parti, bonne chance !");
	}

	this.showQuestion = function (stateParams) {
		$('#answer_status').html("");
		if (stateParams.pic != '') $('#image').html("<img style='max-width: 200px; width:50%' src='" + stateParams.pic + "' />");
		else $('#image').html('');

		$('#question').html(stateParams.question);
		...
		for (var i = 0; i < answers.length; i++) {
			...
			var $div = $("<div>", { answer_id: answerId })
				// add hover style to answers
				.hover(function () {
					$(this).css("background-color", "rgb(255, 255, 162)");
				}, function () {
					$(this).css("background-color", "inherit");
				})
				.attr("style", "font-size:1.25em; margin: 10px; padding: 10px;border-radius: 10px;cursor: pointer;border: 1px solid #ccc; padding: 10px;")
			...
			$div.click(function () {
				if ((userType == 'official_participant' || userType == 'unofficial_participant') && (curState == states.SHOW_QUESTION || curState == states.TEST_QUESTION)) {
					...
					$('#answers div').css("background-color", "inherit");
					$(this).css("background-color", "rgb(255, 255, 162)");
				}
			});
	...
	this.showAnswer = function (stateParams) {
		...
		if (!correctAnswer && selectedAnswerId != false) {
			$('#answers .answer_' + selectedAnswerId).css("background-color", "rgb(255, 162, 162)");
		}

		// change color of correct answer
		$('#answers .answer_' + correctAnswerId).css("border", "2px solid #0f0");

		// reset background color of answers
		$('#answers div').css("background-color", "inherit");

		// remove hover effect
		$('#answers div').unbind('mouseenter mouseleave');

		// change cursor to forbidden
		$('#answers div').css("cursor", "not-allowed");
